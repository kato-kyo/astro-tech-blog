...
### 再利用性設計
このプロジェクトは高い再利用性を実現するため、プロジェクト固有の設定を`src/content/`に集約：

- **プロジェクト固有要素**: `src/content/site/config.yaml`で一元管理
- **ソーシャルリンク**: 設定ファイルから動的生成（enabled/disabledフラグ対応）
- **サイト名・URL**: ハードコーディングを排除し、設定ファイル参照に統一
- **再利用率**: 約98%のソースコードが他プロジェクトで使い回し可能
- **robots.txt**: 設定ファイルから動的生成（静的ファイル不要）
...
- **ソーシャルメディア**: GitHub、Twitter、RSS（enabled/disabledフラグで制御）
...
### robots.txtの動的生成
プロジェクトの完全な再利用性を実現するため、robots.txtは動的生成されます：

- **実装**: `src/pages/robots.txt.ts` - Astro API Route
- **動的要素**: サイトマップURL（`config.yaml`のurl設定から自動生成）
- **メリット**: 新プロジェクトで設定ファイル変更のみで完全対応
- **従来との違い**: 静的な`public/robots.txt`は不要
